...
class AdminKuis extends BaseController
{
   ...
   {
    $referrer = $this->request->getGet('from') ?? 'kuis';
    $seminarModel = new \App\Models\KuisModel();
   ...
   public function delete($kuisId)
   {
    $referrer = $this->request->getGet('from') ?? 'kuis';
       ...
       if ($kuisModel->delete($kuisId)) {
           return redirect()->to('/admin/' . $referrer)->with('success', 'Kuis beserta soal berhasil dihapus.');
       }

       switch ($referrer) {
        case 'dashboard_kuis':
            return redirect()->to('/admin/dashboard_kuis');
        case 'seminar':
