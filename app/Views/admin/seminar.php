...
<body class="bg-[#EEF5FF] h-screen flex">
    ...
    <div class="flex-1 pl-[242px] px-8 py-8 pt-[90px] overflow-auto pb-[50px]">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <!-- Tombol Tambah Data -->
            <div class="mt-4">
                <a href="<?= site_url('admin/tambahSeminar?from=seminar') ?>" class="inline-flex items-center bg-[#176B87] text-white px-4 py-2 rounded shadow ">
                ...
                <table class="w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="text-sm">Judul Acara</th>
                            <th class="text-sm">Penyelenggara</th>
                            <th class="text-sm">Tanggal Acara</th>
                    ...
                    <tbody id="seminarTableBody">
                        ...
                        <tr>
                            ...
                            <td class="text-sm"><?= esc($seminar['judul']) ?></td>
                            <td class="text-sm"><?= esc($seminar['penyelenggara']) ?></td>
                            <td class="text-sm"><?= esc($seminar['tanggal']) ?></td>
                            <td class="text-sm">
                                <span class="<?= $seminar['status'] === 'published' ? 'status-published' : 'status-draft' ?>">
                                    <?= ucfirst($seminar['status']) ?>
                                </span>
                            </td>
                            <td class="flex space-x-3 text-sm">
                                <a href="<?= site_url('admin/editSeminar/' . $seminar['seminar_id'] . '?from=seminar') ?>" 
    ...
    function filterSeminars() {
        ...
        filteredData = seminarsData.filter(seminar => {
            return seminar.judul.toLowerCase().includes(searchQuery) ||
                   seminar.penyelenggara.toLowerCase().includes(searchQuery);
        });
    ...
    function updateTable(data) {
        ...
        const tableBody = document.getElementById('seminarTableBody');
        tableBody.innerHTML = '';

        selectedData.forEach((seminar, index) => {
            const statusClass = seminar.status === 'published' ? 'status-published' : 'status-draft';
            const row = document.createElement('tr');
            
            row.innerHTML = `
                <td class="text-sm">${startIndex + index + 1}</td>
                <td class="text-sm">${seminar.judul}</td>
                <td class="text-sm">${seminar.penyelenggara}</td>
                <td class="text-sm">${seminar.tanggal}</td>
                <td class="text-sm">
